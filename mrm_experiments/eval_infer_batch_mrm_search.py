...
def main():
    ...
    parser.add_argument("--search-type", default="mrm", choices=["rbf", "bon", "smc", "mrm"])
    parser.add_argument("--search-n", default=8, type=int)
    parser.add_argument("--mrm-t1", default=0.5, type=float)
    parser.add_argument("--mrm-t2", default=0.8, type=float)
    parser.add_argument("--reward-url", default=os.environ.get("MRM_REWARD_URL", "http://127.0.0.1:8000/infer"))
    parser.add_argument("--max-utterances", default=None, type=int)
    ...
    os.environ["MRM_REWARD_URL"] = reward_url
    ...
    run_name_parts.extend([
        f"cfg-{cfg_strength:g}",
        f"speed-{speed:g}",
    ])
    if sway_sampling_coef is not None and sway_sampling_coef >= 0:
        run_name_parts.append(f"ss-{sway_sampling_coef:g}")
    if use_truth_duration:
        run_name_parts.append("gt-dur")
    if no_ref_audio:
        run_name_parts.append("no-ref-audio")
    if results_tag:
        run_name_parts.append(f"tag-{results_tag}")

    run_name = "__".join(run_name_parts)
    output_dir = f"{rel_path}/results/mrm_search/{run_name}"
    ...
    with accelerator.split_between_processes(prompts_all) as prompts:
        for prompt in tqdm(prompts, disable=not accelerator.is_local_main_process):
            counter += 1
            if max_utterances is not None and counter > max_utterances:
                break
